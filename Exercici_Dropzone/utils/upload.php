...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/Servidor_2DAW/Exercici_Dropzone/media/'.$_FILES['avatar']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/Servidor_2DAW/Exercici_Dropzone/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    }
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/Servidor_2DAW/Exercici_Dropzone/media/default-avatar.png';
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_files(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/Servidor_2DAW/Exercici_Dropzone/media/'.$name)){
		unlink($_SERVER['DOCUMENT_ROOT'].'/Servidor_2DAW/Exercici_Dropzone/media/'.$name);
		return true;
	}else{
		return false;
	}
